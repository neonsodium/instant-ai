...
def create_directory(base_dir, *sub_dirs) -> dict:
    ...
    """
    directory_path = os.path.join(base_dir, *sub_dirs)
...
def load_task_name(task_dir) -> str:
    task_name_path = os.path.join(task_dir, 'task_name.txt')
    if os.path.exists(task_name_path):
        with open(task_name_path, 'r') as f:
            return f.read().strip()  # Remove any surrounding whitespace
    return None
    
@app.route('/get_projects', methods=['GET','POST'])
def list_tasks():
    base_dir = app.config[Config.PROJECTS_DIR_VAR]
    ...
    try:
        for task_id in os.listdir(base_dir):
            ...
            if os.path.isdir(task_dir):
                # TODO task_name = load_task_name(task_dir)
                tasks.append({"task_id": task_id})
                # TODO tasks.append({"task_id": task_id, "task_name": task_name})
...
def create_new_project():
    ...
    result = create_directory(app.config[Config.PROJECTS_DIR_VAR],new_project_id)
...
def upload_file():
    ...
    project_dir = os.path.join(app.config[Config.PROJECTS_DIR_VAR], project_id)
