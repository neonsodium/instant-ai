...
def async_data_processer(directory_project):
    ...
    output_pre_one_hot = os.path.join(directory_project,filename_pre_one_hot_encoded_data_csv())
...
@celery.task
def async_save_and_process_file(project_dir, file_content, filename):
...
def async_optimised_feature_rank(target_var,target_vars_list,directory_project):
    ...
    optimised_feature_rank(target_var,target_vars_list,file_path_label_encoded_csv,directory_project)
    return {"status": "Feature ranking completed"}

@celery.task
def async_optimised_clustering(directory_project,input_file_path_feature_rank_pkl):
    input_file_path_label_encoded_csv = os.path.join(directory_project,filename_label_encoded_data_csv())
    optimised_clustering(directory_project,input_file_path_label_encoded_csv,input_file_path_feature_rank_pkl)
